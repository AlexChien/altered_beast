...
set :repository,  "git@github.com:AlexChien/altered_beast.git"
...
namespace :deploy do
  ...
  task :after_symlink, :roles => [:web] do
    ## create link for tmp
    run "#{release_path}/script/relink.sh #{shared_path}/shared #{release_path}/tmp #{previous_release} #{release_name} tmp"
